...
genai.configure(api_key="AIzaSyBk2M6CBT8YlWsBPKs4UGlHm35fE_tBB8s")
...
def feedback_to_json(feedback_string):
    # Tách thông tin phản hồi thành các phần riêng biệt bằng cách phân chia theo dấu phẩy và ký tự xuống dòng
    feedback_parts = feedback_string.strip().split('\n\n')
    
    # Tạo một từ điển để lưu trữ thông tin phản hồi
    feedback_dict = {}

    for part in feedback_parts:
        # Tách tên phần và nội dung phản hồi bằng dấu hai chấm
        part_name, part_content = part.split(':', 1)
        
        # Xử lý nội dung phản hồi bằng cách tách thành các dòng và chia thành các cặp key-value
        part_lines = part_content.strip().split('\n')
        part_lines = [line.strip('*').strip() for line in part_lines]
        part_lines = [line.split(':', 1) for line in part_lines]
        
        # Chuyển đổi thành từ điển
        part_dict = {key.strip(): value.strip() for key, value in part_lines}
        
        # Thêm vào từ điển chính
        feedback_dict[part_name.strip()] = part_dict

    # Chuyển đổi từ điển thành JSON và trả về
    return json.dumps(feedback_dict, indent=4)
...
def grade_and_comments(topic,assignment) :
    prompt_part = [
            ...
            Please focus on the following aspects, considering the student's current level:
...
#     return {'score': 6.5, 'feed_back': 'Good job! Keep it up!'}

print(grade_and_comments(topic=topic,assignment=assignment))